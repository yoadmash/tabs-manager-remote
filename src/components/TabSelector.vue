...
const selectConnection = () => {
    ...
    windows_list.value = computed(() => {
        const docObj = props.docs.find(doc => doc.id === data_selected.value.connection.value);
...
const selectWindow = () => {
    ...
    tabs_list.value = computed(() => {
        const windowObj = windows_list.value.value.find(window => window.value === data_selected.value.window.value);
        const tabs_list_arr = [];
...
<template>
    <v-row>
        <v-col :cols="12" :sm="data_selected.connection ? 8 : 12">
            <v-combobox clearable label="Connection" :items="connections" v-model="data_selected.connection"
                :menu-props="{ maxWidth: 100 }" @update:model-value="selectConnection"
                @click:clear="connectionCleared" />
        </v-col>
        <v-col :cols="12" :sm="4">
            <v-combobox v-if="data_selected.connection" clearable label="Window" :items="windows_list.value"
                :menu-props="{ maxWidth: 100 }" v-model="data_selected.window" @update:model-value="selectWindow"
                @click:clear="windowCleared" />
        </v-col>
        <v-col :cols="12">
            <v-combobox v-if="data_selected.window" clearable label="Tab" :items="tabs_list.value"
                :menu-props="{ maxWidth: 100 }" v-model="data_selected.tab" @click:clear="tabCleared"
